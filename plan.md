...
| T05 | 文件遍历与分类 | 阶段2 | P0 | 已完成 | 白名单保留；`health-check-win.txt` 始终直接尝试清空且不删除，即使正在使用但允许写入时也应清空；清空失败计入 `Fail`；当天其他文件保留，历史文件进入处理。 |
...
## 注意事项
所有破坏性测试应使用临时目录。应验证 `Tue 08/11/2026` 可转换为 `20260811`，确认 `cleanup_action.log` 不会被清理，并确认当天的 `health-check-win.txt` 会被清空但仍存在；该 `.txt` 文件正在使用但仍允许写入时，也应清空且不删除；真正无法写入时才应显示 `[FAIL] ... [clear-only]` 并增加 `Fail`。部署后应执行 `schtasks /Query /TN "\WardenLogCleanupTask" /V /FO LIST`，确认“要运行的任务”通过 `cmd.exe /d /c` 和 `/scheduled` 调用已覆盖的部署脚本。修改默认路径、白名单、仅清空文件或任务时间后，应同步更新 `PRD.md` 和 `产品使用说明.md`。
