...
2. `health-check-win.log` 为仅清空文件：每次运行均直接尝试清空内容，绝不删除，也不受最后修改日期为当天的保留规则影响；即使文件正在被使用，只要仍允许写入也会清空。若写入进程以独占方式锁定文件，Windows 会拒绝截断写入，脚本无法绕过该锁定，保留原内容并计入 `Fail`。
3. 最后修改日期为当天的其他文件保留；文件名不参与日期判断。
4. 其他未占用历史文件强制删除。
5. 其他被占用历史文件尝试清空内容，保留文件本身；失败计入 `Fail`。

## 4. 输出与配置
控制台显示手动执行的文件级结果和汇总；定时执行不显示交互输出。仅清空文件清空成功显示 `[CLEAR] health-check-win.log [clear-only]`，失败显示 `[FAIL] health-check-win.log [clear-only]`。汇总追加写入：
...
## 5. 边界
不提供远程部署、归档压缩、强制终止占用进程、强制关闭文件句柄或 GUI。被独占锁定的日志无法在不释放句柄的前提下清空；需等待写入进程释放文件，或由运维人员停止对应服务后手动清空再启动。目标目录不存在或当天日期无法识别时，脚本会提示错误并退出。定时任务注册失败或注册后无法验证时，脚本会输出警告，但仍继续执行本次清理。
